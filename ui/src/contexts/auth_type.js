export default function AuthTypeContext(_component) {
    ...
    this.onViewSectionRendering = (_handle, _config, _section) => {    
        ...
        let _widget = [];
        if (_section === "content" && this.component.currentRecord["auth_type_grid"]) {           
            _widget.push(<Capability roleId={this.component.currentRecord["auth_type_grid"]._id} />);
        }
    ...
    this.saveAuthType = () => {
        ...
        if (authType) {
            ...
            request["method"] = "PUT";
            request["endpoint"] = "/system/auth_type/" + authType._id;
        } else {
            /* It's a new record */
            request["method"] = "POST";
            request["endpoint"] = "/system/auth_type";
        }
