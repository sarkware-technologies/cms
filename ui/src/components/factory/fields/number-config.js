...
const NumberFieldConfig = (props, ref) => {

    const [state, setState] = useState({
        required: ('required' in props.config) ? props.config.required : false,
        default: ('default' in props.config) ? props.config.default : 0,        
    ...
    return (
        <div className="pharmarack-cms-field-configure-view">
            ...
            <div className="pharmarack-cms-field-configure-row">
                ...
                <div className="config">
                    <input type="number" value={state.default} onChange={handleDefaultChange} />
                </div>
            </div>
            <div className="pharmarack-cms-field-configure-row">
                <div className="label">
                    <h2>Minimum</h2>    
                    <p>Checks if the value is less than or equal to the given maximum</p>
                </div>  
                <div className="config">
                    <input type="number" value={state.min} onChange={handleMinChange} />
                </div>                  
            </div>
            <div className="pharmarack-cms-field-configure-row">
                <div className="label">
                    <h2>Maximum</h2>    
                    <p>Checks if the value is greater than or equal to the given maximum</p>
                </div>
                <div className="config">
                    <input type="number" value={state.max} onChange={handleMaxChange} />
                </div>
            </div>
...
export default forwardRef(NumberFieldConfig);