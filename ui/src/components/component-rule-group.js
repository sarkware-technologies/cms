...
const ComponentRuleGroup = (props, ref) => {
    ...
    const renderRules = (_groupKey) => {
        ...
        for (let i = 0; i < groups[_groupKey].length; i++) {
            ...
            if (groups[_groupKey][i]) {
                ...
                items.push(<ComponentRule 
                    ref={ruleRef} 
                    key={uuidv4()} 
                    id={groups[_groupKey][i]._id} 
                    groupKey={_groupKey} 
                    ruleIndex={i} 
                    record={_record} 
                    registerInstance={(_groupKey, _ruleId, _ref) => registerRuleInstance(_groupKey, _ruleId, _ref)} 
                    removeRule={(_groupKey, _rIndex, _ruleId) => removeRule(_groupKey, _rIndex, _ruleId)} />
                );   
            }
...
export default forwardRef(ComponentRuleGroup);